...
class SynzealSpider(BaseSpider):
    ...
    def parse(self, response, **kwargs):
        a_nodes = response.xpath('//a[contains(@class, "clsMaincatdiv")]')
        for a in a_nodes:
    ...
    def list_parse(self, response):
        urls = response.xpath('//h2[@class="product-title"]/a/@href').extract()
        for rel_url in urls:
            yield Request(urljoin(response.url, rel_url),
    ...
    def detail_parse(self, response):
        ...
        en_name = re.sub(r'\r?\n', "", en_name)
        if not en_name:
            en_name = response.xpath('//div[@class="product-name"]/h1/text()').get('')
        ...
        d = {
            ...
            'info1': response.xpath('//b[text()="Synonym: "]/../text()').get(default=""),
            'parent': response.meta.get('parent'),
            'img_url': response.xpath('//div[@class="product-details-tab"]//img/@src').get(),
            'smiles': response.xpath('//b[text()="Smiles: "]/../text()').get(),
        }
        yield RawData(**d)

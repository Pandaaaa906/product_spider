...
class AikonchemSpider(BaseSpider):
    ...
    def get_sign(self, cur_time: int):
        return md5(
            f"PBn2W4%6Y7J4SYWERTYgo0k%n!@d7x%k{cur_time}".encode("utf-8")
        ).hexdigest()
    ...
    def parse(self, response, **kwargs):
        categories = response.xpath('//div[@class="product-tab"]/p[@name!="0"]')
        for p_category in categories:
            cate_id = p_category.xpath("./@name").get()
            cate_name = p_category.xpath("./text()").get()
            if not cate_id:
                continue
            form = {
                "page": "1",
    ...
    def parse_list(self, response):
        ...
        for cas in cas_list:
            ...
            yield self.make_request(
                self.detail_url, form, self.parse_detail, meta=response.meta
            )
    ...
    def parse_detail(self, response):
        ...
        if not product_info:
            self.logger.warning("No product info")
            return

...
class USPSpider(BaseSpider):
    ...
    def _start_requests(self):
        ...
        yield Request(
            f"{self.store_url}?{urlencode(d)}", meta={"data": d}, callback=self.parse
        )
    ...
    def parse(self, response, **kwargs):
        ...
        data["offset"] = offset
        yield Request(
            url=f"{self.store_url}?{urlencode(data)}",
            meta={"data": data},
            callback=self.parse,
        )
    ...
    def parse_typeahead_results(self, response):
        ...
        for record in records:
            ...
            for inner_record in inner_records:
                ...
                # 提取产品名称
                en_name = first(attrs.get("product.displayName", []), None) or first(
                    attrs.get("product.description", []), None
                )
                ...
                parent = first(
                    attrs.get("USPProductType.usp_schedule_b_desc", []), None
                )
                ...
                package_size = first(
                    attrs.get("USPProductType.usp_packing_size", []), None
                )
